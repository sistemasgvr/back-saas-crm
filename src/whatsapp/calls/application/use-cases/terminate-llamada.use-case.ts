...
@Injectable()
export class TerminateLlamadaUseCase {
  constructor(
  ...
  async execute(organizacionId: string, callId: string) {
    ...
    if (llamada.contestadaEn) {
      duracionSeg = Math.max(
        0,
        Math.round((finEn.getTime() - llamada.contestadaEn.getTime()) / 1000),
      );
    }
