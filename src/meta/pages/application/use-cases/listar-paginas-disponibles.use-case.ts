...
/** Páginas del token OAuth de la org que aún NO están vinculadas — para el modal "Agregar página". */
@Injectable()
export class ListarPaginasDisponiblesUseCase {
  constructor(
  ...
  async execute(organizacionId: string) {
    ...
    const [todas, vinculadas] = await Promise.all([
      this.graph.listarPaginas(accessToken),
      this.paginas.listarPageIdsVinculados(organizacionId),
    ]);

    const vinculadasSet = new Set(vinculadas);
    return todas.filter((pagina) => !vinculadasSet.has(pagina.id));
  }
}
